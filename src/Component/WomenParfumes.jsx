...
const Shop = () => {
  ...
  const WomenParfumes = useSelector((state) => state.WomenParfumes);
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 20;
  const totalPages = Math.ceil(WomenParfumes.length / itemsPerPage);

  const startIndex = (currentPage - 1) * itemsPerPage;
  const endIndex = startIndex + itemsPerPage;
  const currentItems = WomenParfumes.slice(startIndex, endIndex);
  ...
  const handleDelete = (id) => {
    dispatch({ type: 'DELETE_WOMEN_PARFUME', payload: id });
  };

  const handleUpdate = (id) => {
    const productToUpdate = WomenParfumes.find(product => product.id === id);
    ...
    if (newName && newPrice) {
      ...
      dispatch({ type: 'UPDATE_WOMEN_PARFUME', payload: { id: productToUpdate.id, data: updatedProduct } });
    }
  };
  ...
  const handleAddProduct = () => {
    ...
    if (newProduct.name && !isNaN(newProduct.price) && newProduct.img && newProduct.category && !isNaN(newProduct.rating)) {
      dispatch({ type: 'ADD_WOMEN_PARFUME', payload: newProduct });
    } else {
  ...
  return (
    <div>
      ...
      <h3>Discover the best fragrances curated just for you!</h3>
