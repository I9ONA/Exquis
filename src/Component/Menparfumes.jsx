...
const Shop = () => {
  ...
  const MenParfumes = useSelector((state) => state.MenParfumes);
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 20;
  const totalPages = Math.ceil(MenParfumes.length / itemsPerPage);

  const startIndex = (currentPage - 1) * itemsPerPage;
  const endIndex = startIndex + itemsPerPage;
  const currentItems = MenParfumes.slice(startIndex, endIndex);
  ...
  const handleDelete = (id) => {
    dispatch({ type: 'DELETE_MEN_PARFUME', payload: id });
  };

  const handleUpdate = (id) => {
    const productToUpdate = MenParfumes.find(product => product.id === id);
    ...
    if (newName && newPrice) {
      ...
      dispatch({ type: 'UPDATE_MEN_PARFUME', payload: { id: productToUpdate.id, data: updatedProduct } });
    }
  };
  ...
  const handleAddProduct = () => {
    ...
    if (newProduct.name && !isNaN(newProduct.price) && newProduct.img && newProduct.category && !isNaN(newProduct.rating)) {
      dispatch({ type: 'ADD_MEN_PARFUME', payload: newProduct });
    } else {
